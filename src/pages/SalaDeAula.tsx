...
export const SalaDeAula: React.FC = () => {
  ...
  const loadClassroomData = async () => {
    ...
    try {
      ...
      const allMatriculas = await api.getMatriculasByUser(currentUser.ID_Usuario);
  ...
  const handleToggleLessonComplete = async (aulaId: number) => {
    ...
    try {
      await api.toggleProgressoAula(currentUser.ID_Usuario, aulaId, !isCompleted);
      ...
      const completedLessons = progData.filter(p => aulas.some(a => a.ID_Aula === p.ID_Aula)).length;
      
      if (completedLessons === totalLessons && totalLessons > 0 && !matricula.DataConclusao) {
        ...
        const updatedMat = await api.getMatriculasByUser(currentUser.ID_Usuario);
        setMatricula(updatedMat.find(m => m.ID_Curso === Number(cursoId)) || null);
      }
    } catch (err) {
  ...
  const handleEmitCertificate = async () => {
    ...
    try {
      const existingCerts = await api.getCertificadosByUser(currentUser.ID_Usuario);
      const alreadyHasCert = existingCerts.some(c => c.ID_Curso === curso.ID_Curso);
      ...
      alert('🏆 Certificado gerado com sucesso! Redirecionando para visualização...');
      navigate('/certificados');
    } catch (err) {
        ...
        <div className="row justify-content-center">
          <div className="col-md-8 col-lg-6">
            <div className="glass-panel p-5 text-center my-5">
              ...
              <h3 className="text-white fw-bold mt-4">Sala de Aula Bloqueada</h3>
              <p className="text-secondary mb-4">
                Você ainda não está matriculado no curso <strong>"{curso?.Titulo}"</strong>. Matricule-se agora gratuitamente para começar a estudar imediatamente!
              </p>
              <button 
  ...
  const isAulaCompleted = (aulaId: number) => progressos.some(p => p.ID_Aula === aulaId);
  
  // Calculate completion percentage
  const totalAulasCount = aulas.length;
  const completedAulasCount = progressos.filter(p => aulas.some(a => a.ID_Aula === p.ID_Aula)).length;
  const progressPercent = totalAulasCount > 0 ? (completedAulasCount / totalAulasCount) * 100 : 0;
    ...
    <div className="container-fluid px-md-5">
            ...
            <div className="d-flex align-items-center gap-4">
              <div style={{ width: '150px' }}>
                <div className="d-flex justify-content-between mb-1">
                  ...
                  <span className="text-white fw-bold fs-8">{Math.round(progressPercent)}%</span>
                </div>
                <div className="progress bg-secondary-subtle" style={{ height: '6px' }}>
                  <div 
                    className="progress-bar" 
                    style={{ width: `${progressPercent}%`, background: 'var(--primary-glow)' }}
                  ></div>
                </div>
              </div>
      ...
      <div className="row g-4">
        ...
        <div className="col-lg-4 col-xl-3">
          <div className="glass-panel p-4 h-100" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            <h5 className="text-white fw-bold mb-3"><i className="bi bi-collection-play"></i> Grade de Aulas</h5>
                      ...
                      return (
                        <button
                          ...
                          className={`list-group-item list-group-item-action bg-transparent border-0 text-white rounded p-2 d-flex align-items-start gap-2 text-start ${
                            isSelected ? 'bg-white bg-opacity-10 fw-semibold text-gradient-primary' : 'text-secondary'
                          }`}
                        >
                          ...
                          >
                            <i className={`bi ${completed ? 'bi-check-circle-fill text-success' : 'bi-circle text-secondary'}`}></i>
                          </span>
                          
                          <div className="flex-grow-1">
                            <span className="fs-7 d-block text-white">{aula.Titulo}</span>
                            <span className="fs-9 text-secondary d-flex align-items-center gap-1 mt-1">
                              <i className={`bi ${
                                aula.TipoConteudo === 'Vídeo' ? 'bi-play-circle' :
                                aula.TipoConteudo === 'Quiz' ? 'bi-question-circle' : 'bi-file-text'
                              }`}></i>
        ...
        <div className="col-lg-8 col-xl-9">
          <div className="glass-panel p-4 h-100 d-flex flex-column justify-content-between" style={{ minHeight: '55vh' }}>
            {selectedAula ? (
              <>
                <div>
                  {/* Lesson Heading Info */}
                  <div className="d-flex justify-content-between align-items-start border-bottom border-secondary pb-3 mb-4 flex-wrap gap-2">
                    <div>
                      <span className="badge bg-primary badge-custom mb-2">{selectedAula.TipoConteudo}</span>
                      <h4 className="text-white fw-bold mb-0">{selectedAula.Titulo}</h4>
                    </div>
                    
                    {/* Mark complete button */}
                    <button
                      onClick={() => handleToggleLessonComplete(selectedAula.ID_Aula)}
                      className={`btn btn-sm ${isAulaCompleted(selectedAula.ID_Aula) ? 'btn-success' : 'btn-outline-custom'}`}
                    >
                      <i className={`bi ${isAulaCompleted(selectedAula.ID_Aula) ? 'bi-check-all' : 'bi-check'}`}></i>
                      {isAulaCompleted(selectedAula.ID_Aula) ? ' Concluída' : ' Marcar como Concluída'}
                    </button>
                  </div>
                  ...
                  <div className="classroom-viewport my-3">
                    {selectedAula.TipoConteudo === 'Vídeo' && (
                      <div className="ratio ratio-16x9 rounded overflow-hidden shadow-lg" style={{ border: '1px solid var(--panel-border)' }}>
                        <iframe
                    ...
                    {selectedAula.TipoConteudo === 'Texto' && (
                      <div className="p-4 bg-black bg-opacity-20 rounded border border-secondary" style={{ lineHeight: '1.8' }}>
                        <p className="text-white fs-6 whitespace-pre-wrap">{selectedAula.URL_Conteudo}</p>
                        <div className="alert alert-info mt-4 fs-8 border-secondary bg-transparent text-secondary">
                          ...
                          Dica acadêmica: Faça resumos em seu caderno ou repositório pessoal de estudos para fixar o aprendizado deste módulo.
                        </div>
                      </div>
                    )}
                    ...
                    {selectedAula.TipoConteudo === 'Quiz' && (
                      <div className="p-4 bg-black bg-opacity-20 rounded border border-secondary">
                        <h6 className="text-white fw-bold mb-3"><i className="bi bi-patch-question-fill text-warning"></i> Questionário de Fixação</h6>
                        <p className="text-secondary fs-7 mb-4">{selectedAula.URL_Conteudo}</p>

                        <div className="d-grid gap-2 mb-4" style={{ maxWidth: '450px' }}>
                          {['A) Utilizar Hooks customizados', 'B) Redux Toolkit apenas', 'C) Props drilling em múltiplos níveis', 'D) Nenhuma das anteriores'].map((option, idx) => (
                            <button
                              key={idx}
                              onClick={() => !quizSubmitted && setQuizAnswer(option)}
                        ...
                        {!quizSubmitted ? (
                          ...
                          <div className="alert alert-success d-flex align-items-center gap-3">
                            ...
                            <div>
                              <strong className="d-block text-white">Resposta Enviada!</strong>
                              <span className="fs-8 text-secondary">Parabéns pela conclusão. Clique no botão de marcar aula como concluída acima para arquivar seu progresso.</span>
                            </div>
                          </div>
                        )}
                      </div>
                ...
                <div className="d-flex justify-content-between align-items-center border-top border-secondary pt-3 mt-4">
                  <span className="text-secondary fs-8">Duração Estimada: {selectedAula.DuracaoMinutos} minutos</span>
                  <span className="text-secondary fs-8">Ordem da Aula no Módulo: #{selectedAula.Ordem}</span>
                </div>
              </>
            ) : (
              <div className="d-flex flex-column justify-content-center align-items-center py-5">
                ...
                <p className="text-secondary">Escolha um módulo para começar sua trilha de estudos acadêmicos.</p>
              </div>
            )}
          </div>
        </div>
