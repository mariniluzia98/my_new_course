...
import type { Curso, Categoria, Modulo, Aula } from '../types';
...
export const Cursos: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'catalogo' | 'conteudo'>('catalogo');
  const [selectedContentCursoId, setSelectedContentCursoId] = useState<number | ''>('');
  ...
  const loadContentData = async (cId: number) => {
    try {
  ...
  const handleDeleteCourse = async (id: number) => {
    if (window.confirm('Deseja excluir este curso? Modulos e aulas não serão excluídos automaticamente no mock JSON Server, mas ficarão ocultos.')) {
      try {
  ...
  const handleAddAula = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedContentCursoId) {
        const targetCurso = cursos.find(c => c.ID_Curso === Number(selectedContentCursoId));
        if (targetCurso) {
          await api.updateCurso(targetCurso.ID_Curso, {
            TotalAulas: targetCurso.TotalAulas + 1,
            TotalHoras: Math.round((targetCurso.TotalHoras * 60 + Number(aulaDuracao)) / 60)
          });
        }
      }
  ...
  const filteredCursos = selectedCategoryFilter === 'todas'
    ? cursos
      ...
      <div className="row mb-4">
        <div className="col-12 d-flex justify-content-between align-items-center flex-wrap gap-2">
          <div>
            ...
            <p className="text-secondary">Gerencie categorias de cursos, listagem acadêmica e monte a estrutura de módulos e aulas.</p>
          </div>
          ...
          <div className="col-lg-4">
            ...
            {isAdminOrInstructor && (
              <div className="glass-panel p-4 mb-4">
                ...
                <form onSubmit={handleSaveCategory}>
                  ...
                  <div className="d-flex gap-2">
                    <button type="submit" className="btn btn-sm btn-gradient-primary flex-grow-1">
                      Salvar
                    </button>
                    {catId && (
                      <button type="button" className="btn btn-sm btn-outline-custom" onClick={() => { setCatId(null); setCatNome(''); setCatDesc(''); }}>
                        Cancelar
                      </button>
                    )}
                  </div>
            ...
            <div className="glass-panel p-4 mb-4">
              ...
              <div className="list-group list-group-flush">
                ...
                >
                  <span><i className="bi bi-tag-fill me-2"></i> Todas as Categorias</span>
                  <span className="badge bg-secondary rounded-pill">{cursos.length}</span>
                </button>
                {categorias.map(cat => {
                  const count = cursos.filter(c => c.ID_Categoria === cat.ID_Categoria).length;
                  return (
                    <div
                    ...
                    >
                      <button
                        onClick={() => setSelectedCategoryFilter(cat.ID_Categoria)}
                      ...
                      >
                        <span><i className="bi bi-tag me-2"></i> {cat.Nome}</span>
                        <span className="badge bg-secondary rounded-pill">{count}</span>
                      </button>
                      {isAdminOrInstructor && (
                        <div className="pe-2 d-flex gap-1">
                          <button onClick={() => { setCatId(cat.ID_Categoria); setCatNome(cat.Nome); setCatDesc(cat.Descricao); }} className="btn btn-sm btn-outline-custom py-0 px-1"><i className="bi bi-pencil fs-8"></i></button>
                          <button onClick={() => handleDeleteCategory(cat.ID_Categoria)} className="btn btn-sm btn-outline-danger py-0 px-1"><i className="bi bi-trash fs-8"></i></button>
                        </div>
                      )}
                    </div>
                  );
            ...
            {isAdminOrInstructor && (
              <div className="glass-panel p-4">
                ...
                <form onSubmit={handleSaveCourse}>
                  <div className="mb-3">
                    <label className="text-secondary fs-8 mb-1">Título do Curso</label>
                    <input
                      type="text"
                  ...
                  <div className="mb-3">
                    <label className="text-secondary fs-8 mb-1">Categoria *</label>
                    <select
                      className="form-select form-select-custom"
                      value={cursoCatId}
                      onChange={(e) => setCursoCatId(e.target.value ? Number(e.target.value) : '')}
                      required
                    >
                  ...
                  <div className="mb-4">
                    <label className="text-secondary fs-8 mb-1">Nível de Dificuldade</label>
                    <select
                      className="form-select form-select-custom"
                      value={cursoNivel}
          ...
          <div className="col-lg-8">
            <div className="glass-panel p-4 h-100">
              <h5 className="text-white fw-bold mb-4">
                <i className="bi bi-collection"></i> Catálogo de Cursos ({filteredCursos.length})
              </h5>

              {filteredCursos.length === 0 ? (
                <div className="text-center py-5">
                  <i className="bi bi-folder2-open fs-1 text-secondary"></i>
                  <p className="text-secondary mt-2">Nenhum curso cadastrado nesta categoria.</p>
                </div>
              ) : (
     ...
     <div className="card bg-black bg-opacity-20 border border-secondary p-3 h-100 d-flex flex-column justify-content-between">
                          <div>
                            <div className="d-flex justify-content-between mb-2">
                              ...
                              <span className={`badge ${c.Nivel === 'Avançado' ? 'bg-danger' : c.Nivel === 'Intermediário' ? 'bg-warning text-dark' : 'bg-success'} text-white`} style={{ fontSize: '0.65rem' }}>
                                {c.Nivel}
                              </span>
                            </div>
                            <h6 className="text-white fw-bold mb-1">{c.Titulo}</h6>
                            <p className="text-secondary fs-8 line-clamp-2" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{c.Descricao}</p>
                          </div>
                          
                          <div className="border-top border-secondary pt-2 mt-2 d-flex justify-content-between align-items-center">
                            <span className="text-secondary fs-8"><i className="bi bi-clock"></i> {c.TotalHoras}h | {c.TotalAulas} Aulas</span>
                            {isAdminOrInstructor && (
                              <div className="d-flex gap-1">
                                ...
                                >
                                  <i className="bi bi-list-nested fs-8"></i> Aulas
                                </button>
                                <button
                                  onClick={() => handleDeleteCourse(c.ID_Curso)}
        ...
        <div className="row g-4">
          <div className="col-lg-4">
            ...
            <div className="glass-panel p-4 mb-4">
              <label className="text-secondary fs-8 fw-semibold mb-2">Selecione o Curso para Editar Conteúdo</label>
              <select
                className="form-select form-select-custom"
                value={selectedContentCursoId}
                onChange={(e) => setSelectedContentCursoId(e.target.value ? Number(e.target.value) : '')}
              >
                <option value="">Selecione...</option>
                {cursos.map(c => (
            ...
            {isAdminOrInstructor && selectedContentCursoId && (
              <>
                ...
                <div className="glass-panel p-4 mb-4">
                  <h6 className="text-white fw-bold mb-3"><i className="bi bi-folder-plus text-primary"></i> Adicionar Módulo</h6>
                  <form onSubmit={handleAddModule}>
                    ...
                    <button type="submit" className="btn btn-sm btn-gradient-primary w-100">Criar Módulo</button>
                  </form>
                </div>

                {/* Formulário para Adicionar Aula */}
                <div className="glass-panel p-4">
                  <h6 className="text-white fw-bold mb-3"><i className="bi bi-file-earmark-plus text-secondary"></i> Adicionar Aula</h6>
                  <form onSubmit={handleAddAula}>
                    <div className="mb-2">
                      <select
                        className="form-select form-select-custom"
                        value={aulaModId}
                        onChange={(e) => setAulaModId(e.target.value ? Number(e.target.value) : '')}
                        required
                      >
                        <option value="">Selecione o Módulo *</option>
                        {modulos.map(m => (
                          <option key={m.ID_Modulo} value={m.ID_Modulo}>Mód. {m.Ordem}: {m.Titulo}</option>
                        ))}
                      </select>
                    </div>
                    <div className="mb-2">
                    ...
                    <div className="mb-2">
                      <input
                        type="text"
                        className="form-control form-control-custom"
                        placeholder={aulaTipo === 'Vídeo' ? 'URL do Youtube Embed' : 'Conteúdo de texto do módulo'}
                        value={aulaUrl}
                    ...
                    <div className="row g-2 mb-3">
                      <div className="col-6">
                        <input
                          ...
                          placeholder="Minutos"
                          value={aulaDuracao}
                          onChange={(e) => setAulaDuracao(Number(e.target.value))}
                          min="1"
                          required
                        />
                      </div>
                    ...
                    <button type="submit" className="btn btn-sm btn-gradient-secondary w-100">Criar Aula</button>
                  </form>
                </div>
              </>
            )}
          ...
          <div className="col-lg-8">
            <div className="glass-panel p-4 h-100">
              ...
              {!selectedContentCursoId ? (
                <p className="text-secondary">Selecione um curso para ver seus módulos e aulas.</p>
              ) : modulos.length === 0 ? (
                <div className="text-center py-5">
                  <i className="bi bi-layout-text-sidebar-reverse fs-1 text-secondary"></i>
                  <p className="text-secondary mt-2">Nenhum módulo criado para este curso.</p>
                </div>
              ) : (
                <div className="accordion" id="accordionModules">
                  {modulos.map((mod) => {
                    const modAulas = aulas.filter(a => a.ID_Modulo === mod.ID_Modulo);
                    return (
                      <div className="accordion-item bg-transparent border-secondary mb-3 rounded overflow-hidden" key={mod.ID_Modulo}>
                        <h2 className="accordion-header" id={`heading-${mod.ID_Modulo}`}>
                          <button
                          ...
                          >
                            <div className="d-flex justify-content-between align-items-center w-100 pe-3">
                              <span>
                                <span className="text-warning fw-bold me-2">Módulo {mod.Ordem}:</span>
                                <strong>{mod.Titulo}</strong>
                              </span>
                              <span className="badge bg-secondary fs-9">{modAulas.length} aulas</span>
                            </div>
                          </button>
                        </h2>
                        ...
                        >
                          <div className="accordion-body bg-black bg-opacity-10 p-3">
                            ...
                            {isAdminOrInstructor && (
                              <div className="d-flex justify-content-end mb-2">
                                <button
                                  ...
                                  onClick={() => handleDeleteModule(mod.ID_Modulo)}
                                >
                            ...
                            {modAulas.length === 0 ? (
                              <p className="text-secondary fs-8 mb-0">Nenhuma aula neste módulo.</p>
                            ) : (
                              <div className="list-group list-group-flush">
                                {modAulas.map(aula => (
                                  <div
                                  ...
                                  >
                                    <div className="d-flex align-items-center gap-2">
                                      <span className="text-secondary fs-8 fw-bold">#{aula.Ordem}</span>
                                      <i className={`bi ${
                                        aula.TipoConteudo === 'Vídeo' ? 'bi-play-btn-fill text-primary' :
                                        aula.TipoConteudo === 'Quiz' ? 'bi-question-circle-fill text-warning' : 'bi-file-text-fill text-info'
                                      }`}></i>
                                      <span className="fs-7">{aula.Titulo}</span>
                                      <span className="text-secondary fs-9">({aula.DuracaoMinutos} min)</span>
                                    </div>
                                    {isAdminOrInstructor && (
                                      <button
