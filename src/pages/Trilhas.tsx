...
import type { Trilha, Categoria, TrilhaCurso, Curso } from "../types";
...
export const Trilhas: React.FC = () => {
  ...
  const [backendTrilhaId, setBackendTrilhaId] = useState<
    number | string | null
  >(null);
  ...
  const handleSaveTrilha = async (e: React.FormEvent) => {
    ...
    try {
      if (backendTrilhaId) {
        await api.updateTrilha(backendTrilhaId, {
          Titulo: titulo,
          Descricao: descricao,
          ID_Categoria: Number(catId),
        });
      } else {
        await api.createTrilha({
          Titulo: titulo,
          Descricao: descricao,
          ID_Categoria: Number(catId),
        });
  ...
  const handleDeleteTrilha = async (id: number | string) => {
    if (window.confirm("Excluir esta trilha?")) {
      try {
        await api.deleteTrilha(id);
        if (selectedTrilhaId === trilhaId) setSelectedTrilhaId(null);
        setBackendTrilhaId(null);
        loadData();
      } catch (err) {
        console.error(err);
  ...
  const handleVincularCurso = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const exists = trilhasCursos.some(
        (tc) =>
          tc.ID_Trilha === selectedTrilhaId &&
          tc.ID_Curso === Number(vincCursoId),
      );
  ...
  const handleDesvincularCurso = async (cursoId: number) => {
    ...
    if (
      window.confirm("Tem certeza que deseja remover este curso da trilha?")
    ) {
      try {
      ...
      <div className="row mb-4">
        <div className="col-12">
          ...
          <p className="text-secondary">
            Crie trilhas de conhecimento integrando diversos cursos em uma
            jornada sequencial.
          </p>
        </div>
      </div>
          ...
          {isAdminOrInstructor && (
            <div className="glass-panel p-4 mb-4">
              <h5 className="text-white fw-bold mb-3">
                {trilhaId ? "Editar Trilha" : "Nova Trilha de Conhecimento"}
              </h5>
                    ...
                    >
                      ...
                      >
                        <span>
                          ...
                          {cat && (
                            <span
                              className="badge bg-secondary-subtle text-secondary ms-2"
                      ...
                      {isAdminOrInstructor && (
                        <div className="pe-2 d-flex gap-1">
                          <button
                            onClick={() => {
                              setTrilhaId(t.ID_Trilha);
                              setBackendTrilhaId(t.id);
                              setTitulo(t.Titulo);
                              setDescricao(t.Descricao);
                              setCatId(t.ID_Categoria);
                            }}
          ...
          <div className="glass-panel p-4 h-100">
            {activeTrilha ? (
              <>
                <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 border-bottom border-secondary pb-3 mb-4">
                  <div>
                    <span className="badge bg-secondary-subtle text-secondary badge-custom mb-2">
                      {categorias.find(
                        (c) => c.ID_Categoria === activeTrilha.ID_Categoria,
                      )?.Nome || "Geral"}
                    </span>
                    <h3 className="text-white fw-bold m-0">
                      {activeTrilha.Titulo}
                    </h3>
                    <p className="text-secondary fs-7 mt-1 mb-0">
                      {activeTrilha.Descricao}
                    </p>
                  </div>
                </div>
                ...
                {/* Timeline Roadmap */}
                <h5 className="text-white fw-bold mb-3">
                  <i className="bi bi-signpost-split"></i> Mapa da Jornada
                </h5>
                ...
                {activeTrilhaCursos.length === 0 ? (
                  <div className="text-center py-5">
                    ...
                    <p className="text-secondary mt-2">
                      Nenhum curso associado a esta trilha ainda.
                    </p>
                  </div>
                ) : (
                      ...
                      return (
                        <div className="timeline-item-trail" key={tc.id}>
                          <div className="glass-panel p-3 d-flex justify-content-between align-items-center">
                            <div>
                              <span className="text-warning fw-bold fs-8 d-block mb-1">
                                CURSO {tc.Ordem}
                              </span>
                            ...
                            {isAdminOrInstructor && (
                              <button
                                onClick={() =>
                                  handleDesvincularCurso(curso.ID_Curso)
                                }
              ...
              <p className="text-secondary">
                Selecione uma trilha para gerenciar e visualizar a jornada.
              </p>
            )}
          </div>
        </div>
