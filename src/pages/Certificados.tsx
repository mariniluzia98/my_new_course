...
import type { Certificado, Usuario } from "../types";
...
export const Certificados: React.FC = () => {
  ...
  const [validatedStudent, setValidatedStudent] = useState<Usuario | null>(
    null,
  );
  ...
  const handleValidateCode = async (e: NewType) => {
    e.preventDefault();
      ...
      <div className="row mb-4">
        <div className="col-12">
          ...
          <p className="text-secondary">
            Emita certificados de conclusão, visualize suas conquistas e
            verifique a validade de certificados emitidos.
          </p>
        </div>
      </div>
      ...
      <div className="row g-4">
        ...
        <div className="col-lg-5">
          ...
          <div className="glass-panel p-4 mb-4">
            <h5 className="text-white fw-bold mb-3">
              <i className="bi bi-shield-fill-check text-warning"></i> Validador
              de Certificados
            </h5>
            <p className="text-secondary fs-8">
              Insira o código de verificação para checar a validade do
              certificado emitido pela plataforma Curso_DEV.
            </p>
                  ...
                  <div className="alert alert-success d-flex align-items-start gap-2 bg-success bg-opacity-10 border-success text-white">
                    ...
                    <div>
                      ...
                      <span className="fs-8 text-secondary d-block mt-1">
                        ...
                        {validatedStudent?.NomeCompleto || "Desconhecido"}
                        <br />
                        <strong>Curso:</strong>{" "}
                        {validatedCourse?.Titulo || "Desconhecido"}
                        <br />
                        <strong>Emissão:</strong>{" "}
                        {new Date(validatedCert.DataEmissao).toLocaleDateString(
                          "pt-BR",
                        )}
                      </span>
                    </div>
                  ...
                  <div className="alert alert-danger d-flex align-items-start gap-2 bg-danger bg-opacity-10 border-danger text-white">
                    ...
                    <div>
                      ...
                      <span className="fs-8 text-secondary mt-1 d-block">
                        Este código não corresponde a nenhum certificado emitido
                        pela plataforma. Verifique caracteres maiúsculos ou
                        hífenes.
                      </span>
                    </div>
                  </div>
          ...
          <div className="glass-panel p-4">
            <h5 className="text-white fw-bold mb-3">
              <i className="bi bi-award"></i> Suas Conquistas (
              {certificados.length})
            </h5>
            {certificados.length === 0 ? (
              <div className="text-center py-4">
                ...
                <p className="text-secondary fs-7">
                  Você ainda não emitiu nenhum certificado. Conclua 100% de um
                  curso para desbloquear.
                </p>
              </div>
            ) : (
              <div className="list-group list-group-flush gap-1">
                {certificados.map((c) => {
                  const course = cursos.find(
                    (cur) => cur.ID_Curso === c.ID_Curso,
                  );
                  const isSelected =
                    selectedCert?.ID_Certificado === c.ID_Certificado;
                  return (
                    <button
                      ...
                      className={`list-group-item list-group-item-action bg-transparent border-0 rounded text-start p-3 ${
                        isSelected
                          ? "bg-white bg-opacity-10 text-gradient-primary"
                          : "text-secondary"
                      }`}
                    >
                      ...
                      <span className="text-secondary fs-8 font-monospace d-block">
                        {c.CodigoVerificacao}
                      </span>
                    </button>
                  );
        ...
        <div className="col-lg-7">
          {selectedCert ? (
            <div>
              <h5 className="text-white fw-bold mb-3">
                <i className="bi bi-eye"></i> Visualização de Certificado
              </h5>
              <CertificateCard
                certificado={selectedCert}
                studentName={currentUser?.NomeCompleto || ""}
                courseTitle={
                  cursos.find((c) => c.ID_Curso === selectedCert.ID_Curso)
                    ?.Titulo || ""
                }
            ...
            <div className="glass-panel p-5 h-100 d-flex flex-column justify-content-center align-items-center text-center">
              ...
              <p className="text-secondary fs-7">
                Comprove suas competências técnicas compartilhando o código de
                autenticidade.
              </p>
            </div>
          )}
