...
export const api = {
  ...
  getUsuarioById: (id: number) => client.get<Usuario>(`/usuarios/${id}`).then(r => r.data),
  createUsuario: (data: Omit<Usuario, 'id' | 'ID_Usuario' | 'DataCadastro'>) => {
    const id = generateId();
  ...
  deleteUsuario: (id: number) => client.delete(`/usuarios/${id}`).then(() => id),
  ...
  getCursoById: (id: number) => client.get<Curso>(`/cursos/${id}`).then(r => r.data),
  createCurso: (data: Omit<Curso, 'id' | 'ID_Curso' | 'DataPublicacao' | 'TotalAulas' | 'TotalHoras'>) => {
    const id = generateId();
  ...
  updateCurso: (id: number, data: Partial<Curso>) => 
    client.patch<Curso>(`/cursos/${id}`, data).then(r => r.data),
  deleteCurso: (id: number) => client.delete(`/cursos/${id}`).then(() => id),
  ...
  updateModulo: (id: number, data: Partial<Modulo>) => 
    client.patch<Modulo>(`/modulos/${id}`, data).then(r => r.data),
  deleteModulo: (id: number) => client.delete(`/modulos/${id}`).then(() => id),
  ...
  updateAula: (id: number, data: Partial<Aula>) => 
    client.patch<Aula>(`/aulas/${id}`, data).then(r => r.data),
  deleteAula: (id: number) => client.delete(`/aulas/${id}`).then(() => id),

  // --- MATRICULAS ---
  getMatriculas: () => client.get<Matricula[]>('/matriculas').then(r => r.data),
  getMatriculasByUser: (usuarioId: number) => 
    client.get<Matricula[]>(`/matriculas?ID_Usuario=${usuarioId}`).then(r => r.data),
  createMatricula: (usuarioId: number, cursoId: number) => {
    ...
    return client.post<Matricula>('/matriculas', newMatricula).then(r => r.data);
  },
  concluirMatricula: (id: number) => 
    client.patch<Matricula>(`/matriculas/${id}`, { DataConclusao: new Date().toISOString() }).then(r => r.data),

  // --- PROGRESSO AULAS ---
  getProgressoAulas: () => client.get<ProgressoAula[]>('/progresso_aulas').then(r => r.data),
  getProgressoByUser: (usuarioId: number) => 
    client.get<ProgressoAula[]>(`/progresso_aulas?ID_Usuario=${usuarioId}`).then(r => r.data),
  toggleProgressoAula: (usuarioId: number, aulaId: number, concluido: boolean) => {
    ...
    if (concluido) {
      ...
      return client.post<ProgressoAula>('/progresso_aulas', newProgresso)
        .then(r => r.data)
        .catch(err => {
          // If already exists, return current
          if (err.response && err.response.status === 500) {
            return client.get<ProgressoAula>(`/progresso_aulas/${idStr}`).then(r => r.data);
          }
          throw err;
        });
    } else {
  ...
  // --- AVALIACOES ---
  getAvaliacoes: () => client.get<Avaliacao[]>('/avaliacoes').then(r => r.data),
  getAvaliacoesByCurso: (cursoId: number) => 
    client.get<Avaliacao[]>(`/avaliacoes?ID_Curso=${cursoId}`).then(r => r.data),
  createAvaliacao: (data: Omit<Avaliacao, 'id' | 'ID_Avaliacao' | 'DataAvaliacao'>) => {
    const id = generateId();
  ...
  updateTrilha: (id: number, data: Partial<Trilha>) => 
    client.patch<Trilha>(`/trilhas/${id}`, data).then(r => r.data),
  deleteTrilha: (id: number) => client.delete(`/trilhas/${id}`).then(() => id),

  // --- TRILHAS CURSOS ---
  getTrilhasCursos: () => client.get<TrilhaCurso[]>('/trilhas_cursos').then(r => r.data),
  vincularCursoTrilha: (trilhaId: number, cursoId: number, ordem: number) => {
    ...
    return client.post<TrilhaCurso>('/trilhas_cursos', newTC).then(r => r.data);
  },
  ...
  // --- CERTIFICADOS ---
  getCertificados: () => client.get<Certificado[]>('/certificados').then(r => r.data),
  getCertificadosByUser: (usuarioId: number) => 
    client.get<Certificado[]>(`/certificados?ID_Usuario=${usuarioId}`).then(r => r.data),
  validarCertificado: (codigo: string) => 
    client.get<Certificado[]>(`/certificados?CodigoVerificacao=${codigo}`).then(r => r.data),
  emitirCertificado: (data: Omit<Certificado, 'id' | 'ID_Certificado' | 'CodigoVerificacao' | 'DataEmissao'>) => {
    const id = generateId();
    ...
    return client.post<Certificado>('/certificados', newCert).then(r => r.data);
  },
  ...
  // --- ASSINATURAS ---
  getAssinaturas: () => client.get<Assinatura[]>('/assinaturas').then(r => r.data),
  createAssinatura: (usuarioId: number, planoId: number, duracaoMeses: number) => {
    const id = generateId();
    ...
    return client.post<Assinatura>('/assinaturas', newAssinatura).then(r => r.data);
  },

  // --- PAGAMENTOS ---
  getPagamentos: () => client.get<Pagamento[]>('/pagamentos').then(r => r.data),
  createPagamento: (assinaturaId: number, valor: number, metodo: string) => {
    ...
    return client.post<Pagamento>('/pagamentos', newPagamento).then(r => r.data);
  },
};
